...
`~sbpy.spectroscopy` provides routines for the modeling and analysis of
emission spectra of gas in comets and reflection spectra of asteroid surfaces,
and color-magnitude conversions.  Sub-modules `sun` and `vega` control
`sbpy`'s photometric calibration.
