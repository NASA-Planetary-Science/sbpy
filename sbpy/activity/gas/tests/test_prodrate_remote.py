...
def mock_nonlte(monkeypatch):
    ...
    def mock_cdensity(*args, **kwargs):
        ...
        if integrated_flux == 0.26 * u.K * u.km / u.s:
            return u.Quantity([2.17469686e14], 1 / u.cm**2)
        elif integrated_flux == 0.27 * u.K * u.km / u.s:
            return u.Quantity([2.25833905e14], 1 / u.cm**2)
        elif integrated_flux == 0.28 * u.K * u.km / u.s:
            return u.Quantity([2.34198124e14], 1 / u.cm**2)
...
def test_remote_prodrate_simple_hcn():
    ...
    for i in range(0, 28):
        ...
        ephemobj = Ephem.from_horizons(
            target, epochs=time, id_type="designation", closest_apparition=True
        )
...
def test_einstein():
    ...
    err = abs(
        (np.array(catalog_result) - np.array(result)) / np.array(catalog_result) * 100
    )
...
def test_Haser_prodrate():
    ...
    mol_data.apply([1.0] / (u.m * u.m), name="cdensity")
...
def test_Haser_pyradex():
    ...
    au = einstein_coeff(mol_data)
    mol_data.apply([au.value] * au.unit, name="eincoeff")
    mol_data.apply([1.0] * u.AU**2 * u.s, name="beta")
    mol_data.apply([1.0] / u.m**2, name="cdensity")
    mol_data.apply([1.0], name="total_number")
...
def test_betafactor_case():
    ...
    beta = beta_factor(mol_data, "H92", ephemobj)
...
def test_pyradex_case():
    ...
    mol_tag = 29002
    cdensity_guess = 1.89 * 10.0 ** (14) / (u.cm * u.cm)
    temp_estimate = 20.0 * u.K
    temp_back = 2.8 * u.K
    ...
    nonLTE = NonLTE()
    cdensity = nonLTE.from_pyradex(
        1.234 * u.K * u.km / u.s, mol_data, iter=100, collider_density={"H2": 900}
    )
