...
class TestOOTransform:
    def test_oo_transform_kep(self):
        ...
        for k in elements:
            assert u.isclose(orbit[k][0], kep_orbit[k][0])
    ...
    def test_oo_transform_com(self):
        ...
        for k in elements:
            assert u.isclose(orbit[k][0], com_orbit[k][0])
        assert u.isclose(orbit["epoch"][0].utc.jd, com_orbit["epoch"][0].utc.jd)
        assert u.isclose(orbit["Tp"][0].utc.jd, com_orbit["Tp"][0].utc.jd)
...
class TestOOPropagate:
    ...
    def test_oo_propagate(self):
        ...
        elements = ["a", "e", "i", "Omega", "w", "M"]
        for k in elements:
            assert u.isclose(oo_orbit[k][0], future_orbit[k][0])
...
COMETS = {
    ...
    "epoch": Time([2459696.55177132, 2459696.55177132], scale="tdb", format="jd"),
...
class TestOrbitTisserandDCriterion:
    def testTisserand(self):
        ...
        assert u.allclose(
            comets.tisserand(jupiter), u.Quantity([2.82130706, 2.79709686])
        )

    @pytest.mark.parametrize(
        "e, q, w, Omega, i, D_SH, D_D",
        (
            (
                0.7457,
                0.95 * u.au,
    ...
    def testDCriterion(self, e, q, w, Omega, i, D_SH_test, D_D_test):
        ...
        orb = Orbit.from_dict({"e": e, "q": q, "w": w, "Omega": Omega, "incl": i})
    ...
    def testDCriterion(self):
        ...
        comets = Orbit.from_dict(COMETS)
        assert u.isclose(comets[0].D_criterion(comets[1]), u.Quantity(0.15598291))
        assert u.isclose(
            comets[0].D_criterion(comets[1], version="d"), u.Quantity(0.0502142)
        )
        assert u.isclose(
            comets[0].D_criterion(comets[1], version="h"), u.Quantity(0.15560596)
        )
        halley = Orbit.from_dict(HALLEY)
        assert u.allclose(
            halley.D_criterion(comets), u.Quantity([2.24077662, 2.50705989])
        )
        assert u.allclose(
            halley.D_criterion(comets, version="d"), u.Quantity([1.14268444, 1.1268579])
        )
        assert u.allclose(
            halley.D_criterion(comets, version="h"),
