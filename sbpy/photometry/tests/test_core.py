...
class TestHG:
    ...
    def test_fit(self):
        pha = np.array(
          [  0.        ,   6.31578947,  12.63157895,  18.94736842,
    ...
    def test_mag(self):
        ...
        mag1_test = np.array(
            [6.773181346311468, 7.746327508868813,
             8.297741273359549, 8.813984838536113, 9.366879943505342,
             10.024055427421063, 10.886692329621765, 12.143261499943726,
             14.18326309145893, 18.48388800989832]) * u.mag
        eph1 = ceres.mag(eph_test)
        eph2 = ceres.mag(eph_dict)
        assert set(eph1.column_names) == {'alpha', 'delta', 'mag', 'r'}
        ...
        mag2_test = np.array(
            [3.34, 4.313146162557345,
             4.864559927048081, 5.380803492224645, 5.9336985971938745,
             6.590874081109595, 7.453510983310297, 8.710080153632259,
             10.750081745147462, 15.050706663586855]) * u.mag
        eph3 = ceres.mag(pha_test)
    ...
    def test_ref(self):
        ...
        ref2_test = np.array(
            [2.87222512e-02, 1.17208744e-02, 7.05333491e-03,
             4.38426786e-03, 2.63474772e-03, 1.43836413e-03,
             6.49851437e-04, 2.04261452e-04, 3.12022404e-05,
             5.94204329e-07])*u.dimensionless_unscaled
        ref2_norm_test = np.array(
            [1.00000000e+00, 4.08076452e-01,
             2.45570407e-01, 1.52643601e-01, 9.17319364e-02, 5.00783911e-02,
             2.26253657e-02, 7.11161011e-03, 1.08634383e-03, 2.06879441e-05])
    ...
    def test_fit(self):
        ...
        data = np.array(
          [ 7.14679706,  7.32220201,  7.85637226,  7.98824651,  8.2029765 ,
           ...
           10.51021594, 10.63215313, 11.15570421, 11.44890748, 11.43888611]) * u.mag
        m0 = HG1G2()
        m = m0.fit(pha, data)
